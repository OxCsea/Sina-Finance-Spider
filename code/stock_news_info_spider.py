...
def get_compNewsInfo(stock_code,stock_name,short_name,stock_exchange):
    page = 1
...
def get_indryNewsInfo(industryCode,compDf):
    '''
    industryCode--行业号码
    ...
    for index, row in compDf.iterrows():
        ...
        resDf_comp = get_compNewsInfo(stock_code,stock_name,short_name,stockExchange)
...
#core function
def get_allNewsInfo(filepath,savePath):
    '''
    ...
    while(index < indlen):
        ...
        resDf_indry = get_indryNewsInfo(industryCode,compDf)
        flag = index
...
#     get_allNewsInfo('companyIndex.csv','news.json')