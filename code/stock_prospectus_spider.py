...
log_name = '../logs/crawlProspectus.log'
...
def crawl_stock_prospectus(input_file, download_path):
    ...
    for ind, ind_data in all_data.groupby('industry_code'):
        ...
        for stock in ind_data.itertuples(index=False):
            ...
            if os.path.exists(file_name):
                ...
                get_Prosepctus(ind, stock.stock_code, ind_path)
        logger.info(f'Finish crawling industry: {ind}')
        count += 1


def get_Prosepctus(ind, stock_code, download_path):
    title, link = get_DownloadURL(ind, stock_code)
    if os.path.exists(f'{download_path}{stock_code}_{title}.pdf'):
        logger.info(f'Have crawn prospectus for stock: {stock_code}')
        return
    if title and link:
        download_file(title, link, stock_code, download_path)


def get_DownloadURL(ind, stock_code):
...
def download_file(title, link, stock_code, download_path):
    ...
    try:
        ...
        download_prospectus_pdf(download_path, f'{stock_code}_{title}', download_link)
    except IndexError:
...
def download_prospectus_pdf(download_path, title, link):
...
if __name__ == '__main__':
    input_file = '../data/company.csv'
    download_path = '../data/prospectus/'
    generatePath(download_path)
    crawl_stock_prospectus(input_file, download_path)